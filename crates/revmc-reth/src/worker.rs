...
impl CompilationWorker {
    ...
    fn worker_loop(
        ...
        // Create a thread-local LLVM context and leak it so compiled function ownership can
        // safely outlive a single compile call.
    ...
    fn compile_bytecode(
        ...
        let result = (|| -> Result<(EvmCompilerFn, usize), revmc::Error> {
            ...
            // SAFETY: JIT-compiled function pointers are owned by the compiler/module.
            // Keep the compiler alive for process lifetime so cached pointers remain valid.
            let _compiler_keepalive = Box::leak(Box::new(compiler));
